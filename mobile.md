...
All three channels start on: consent for them is given by accepting the terms at
sign-up rather than by a separate opt-in (SO20-3139). Withdrawing any of them is
always available.
...
consent on — it is the member's to withdraw — but nothing is sent until the
